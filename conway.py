...
while True: # main program loop
    print('\n\n\n\n\n') # separate each steps with lines
    currentCells = copy.deepcopy(nextCells)

    # Print cells on the screen:
    for y in range(HEIGHT):
        for x in range(WIDTH):
            print(currentCells[x][y], end='') # print the hash or space
        print()
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            ...
            # '% WIDTH' ensures left coordinates is always between 0 and -1
            leftCoord = (x - 1) % WIDTH
           ...
           # Set cells based on Conway's Game of Life Rules:
            if currentCells[x][y] == '#' and (numNeighbors == 2 or numNeighbors == 3):
                ...
                # Everything else dies or stay ahead
                nextCells[x][y] = ' '
